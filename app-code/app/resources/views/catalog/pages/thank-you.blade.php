...
@section('content')
    ...
    <section class="thank-you"
        ...
        <div class="container relative py-16 md:py-24 lg:py-28">
            <h1 class="text-white">{{ __('catalog/pages/thank-you.heading') }}</h1>

            <div class="mt-10 grid gap-12 lg:mt-16 lg:grid-cols-2 lg:gap-x-16 2xl:gap-x-24">
                <div class="min-w-0">
                    <div class="flex flex-col gap-6">
                        <p class="text-lg text-light-gray md:text-2xl">
                            {{ __('catalog/pages/thank-you.labels.order_number', ['number' => $thank_you_data['order_number'] ?? '—']) }}
                        </p>
                    ...
                    <div class="mt-8" data-thank-you-products>
                        @foreach($visible_products as $product)
                            @include('catalog.partials.thank-you.product-item', ['product' => $product])
                        @endforeach
                            ...
                            <div class="accordion group" data-thank-you-products-accordion>
                                <div class="accordion-item">
                                    <div
                                        ...
                                        role="region"
                                        aria-label="{{ __('catalog/pages/thank-you.labels.additional_products') }}"
                                    >
                                    ...
                                    >
                                        <span class="group-[.active]:hidden">
                                            {{ __('catalog/pages/thank-you.buttons.show_all_products', ['count' => count($products)]) }}
                                        </span>
                                        <span class="hidden group-[.active]:inline">
                                            {{ __('catalog/pages/thank-you.buttons.hide_products') }}
                                        </span>
                ...
                <div class="min-w-0 lg:pt-16">
                    <dl class="flex flex-col gap-5 text-base text-white md:text-lg">
                        <div class="grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] md:gap-6">
                            <dt>{{ __('catalog/pages/thank-you.labels.payment_method') }}</dt>
                            <dd class="wrap-break-word">{{ $summary['payment_method'] ?? '—' }}</dd>
                        </div>
                        <div class="grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] md:gap-6">
                            <dt>{{ __('catalog/pages/thank-you.labels.delivery_method') }}</dt>
                            <dd class="wrap-break-word">{{ $summary['delivery_method'] ?? $delivery['method'] ?? '—' }}</dd>
                        </div>
                        <div class="grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] md:gap-6">
                            <dt>{{ __('catalog/pages/thank-you.labels.delivery_address') }}</dt>
                        ...
                        <div class="mt-2 grid gap-2 border-t border-light-gray/40 pt-5 md:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] md:gap-6">
                            <dt>{{ __('catalog/pages/thank-you.labels.subtotal') }}</dt>
                            <dd class="wrap-break-word">{{ $summary['subtotal'] ?? '—' }}</dd>
                        </div>
                        <div class="grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] md:gap-6">
                            <dt>{{ __('catalog/pages/thank-you.labels.packaging') }}</dt>
                            <dd class="wrap-break-word">{{ $summary['packaging'] ?? '—' }}</dd>
                        </div>
                        <div class="grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] md:gap-6">
                            <dt>{{ __('catalog/pages/thank-you.labels.delivery_cost') }}</dt>
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-4 border-t border-light-gray/40 pt-5 text-xl font-bold md:text-2xl">
                            <dt>{{ __('catalog/pages/thank-you.labels.total') }}</dt>
                    ...
                    <div class="mt-8 flex max-w-xl flex-col gap-4 text-base text-light-gray md:text-lg">
                        <p>{{ $summary['notes'] ?? __('catalog/pages/thank-you.fallbacks.notes') }}</p>
                        <p>{{ __('catalog/pages/thank-you.fallbacks.tracking') }}</p>
                        <p>{{ __('catalog/pages/thank-you.fallbacks.contact') }}</p>
                    </div>
                </div>
            </div>

            <x-catalog::common.img
                class="object-contain pointer-events-none absolute left-0 bottom-0 opacity-25"
