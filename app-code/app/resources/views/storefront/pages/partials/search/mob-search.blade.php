    ...
    <div class="drawer-header flex-col items-stretch pt-38px px-4 pb-6">
        <div class="flex items-center justify-between gap-x-2 mb-4">
            <button class="size-8 btn btn-text btn-circle"
                    ...
                    aria-expanded="false"
                    aria-controls="{{ __('storefront/default.aria_labels.back_to_main_mob_menu') }}"
            ...
            <button class="size-8 btn btn-text btn-circle"
                    type="button"
                    aria-expanded="true"
                    aria-controls="{{ __('storefront/default.aria_labels.close_mob_search') }}"
        ...
        <div class="flex flex-col gap-y-1 border-y border-y-opacity-light-gray-40% py-2">
            <form class="mob-search-form _needs-validation"
                  novalidate
                  data-ajax-search-url="{{ localized_route('localized.catalog.live-search-product-ajax.index') }}"
                  action="{{ localized_route('localized.catalog.search-products.index') }}"
                  method="GET">
                <div class="flex space-x-4">
                    ...
                    <input class="mob-search-input w-full border-none"
                           ...
                           name="keyword"
                           type="search"
                           placeholder="{{ __('storefront/default.placeholders.search') }}"
                           required
                           minlength="3"
                           aria-label="{{ __('storefront/default.placeholders.search') }}"/>
                </div>

                <span class="_error">
                    {{ __('storefront/default.errors.keyword_min') }}
                </span>
            </form>
        </div>
    </div>
