...
namespace App\Filament\Resources\ApplicationSettings\OrderStatuses\Tables;
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('descriptions')
                    ->label(__('admin/settings/order_statuses.columns.name'))
                    ->state(fn (OrderStatuses $record): string => $record->descriptions
                        ->pluck('name')
                        ->filter()
                        ->implode(' / '))
                    ->limit(100),
            ...
            ->toolbarActions([
                BulkActionGroup::make([
                    BulkAction::make('activate')
                        ->label(__('admin/settings/order_statuses.actions.activate'))
                        ->icon(Heroicon::CheckCircle)
                        ->action(function (Collection $records): void {
                            self::runBulkAction(
                                __('admin/settings/order_statuses.notifications.activated'),
                                function () use ($records): void {
                                    app(OrderStatusManagementService::class)->activate($records);
                                },
                            );
                        })
                    ...
                    BulkAction::make('deactivate')
                        ->label(__('admin/settings/order_statuses.actions.deactivate'))
                        ->icon(Heroicon::XCircle)
                        ->action(function (Collection $records): void {
                            self::runBulkAction(
                                __('admin/settings/order_statuses.notifications.deactivated'),
                                function () use ($records): void {
                                    app(OrderStatusManagementService::class)->deactivate($records);
                                },
                            );
                        })
                        ->deselectRecordsAfterCompletion(),
