...
class CompositionAndCareTabSchema
{
    /**
    ...
    {
        return Tab::make(__('admin/catalogs/products/products.tabs.composition_and_care'))
            ->schema([
                Section::make(__('admin/catalogs/products/products.sections.composition_and_care'))
                    ->schema([
                        Tabs::make('ProductCompositionAndCareLanguageTabs')
    ...
    private static function buildLanguageTabs(Collection $active_languages): array
    {
        return $active_languages
            ->map(function (Language $language): Tab {
                $language_path = "composition_and_care_data.translations.$language->id";

                return Tab::make((string) $language->name)
                    ->badge((string) $language->code)
                    ->schema([
                        Section::make(__('admin/catalogs/products/products.sections.composition_block'))
                            ->schema([
                                TextInput::make("$language_path.composition.title")
                                    ->label(__('admin/catalogs/products/products.labels.composition_title'))
                                    ->default(__('catalog/default.product.details.composition'))
                                    ->maxLength(255)
                                    ->columnSpanFull(),

                                Repeater::make("$language_path.composition.items")
                                    ->label(__('admin/catalogs/products/products.labels.composition_items'))
                                    ->schema([
                                        TextInput::make('value')
                                            ->label(__('admin/catalogs/products/products.labels.composition_item_value'))
                                            ->maxLength(500)
                                            ->columnSpanFull(),
                                    ])
                                    ->defaultItems(0)
