...
class CategoryController extends Controller
{
    ...
    public function show(
        ...
        $data = [
            ...
            ...$response_data,
            'products' => $this->prepareProductsForView((array) Arr::get($response_data, 'products', [])),
        ];
     ...
     */
    private function prepareProductsForView(array $products): array
    {
        return collect($products)
            ->values()
            ->map(function (array $product, int $index): array {
                $quantity  = (int) Arr::get($product, 'quantity', 0);
                $price     = (string) Arr::get($product, 'price.formatted', '');
                $old_price = (string) Arr::get($product, 'price.base_formatted', '');

                return [
                    'url'             => (string) Arr::get($product, 'url', ''),
                    'name'            => (string) Arr::get($product, 'name', ''),
                    'image_urls_data' => (array) Arr::get($product, 'image_data.urls', []),
                    'image_width'     => (int) Arr::get($product, 'image_data.width', 420),
                    'image_height'    => (int) Arr::get($product, 'image_data.height', 420),
                    'price'           => $price,
                    'old_price'       => $old_price,
                    'has_discount'    => filled(Arr::get($product, 'price.discount_value'))
                        && $old_price !== ''
                        && $old_price !== $price,
                    'stock_text' => $quantity > 0
                        ? (string) __('catalog/default.texts.category_products_in_stock', ['quantity' => $quantity])
                        : '',
                    'is_mixed_layout' => (($index + 1) % 7 === 0),
                ];
            })
            ->all();
    }
