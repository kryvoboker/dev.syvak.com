...
final readonly class FailurePageService
{
        ...
        try {
            ...
            return [
                ...
                'recovery' => [
                    ...
                    'retry_count' => (int) Arr::get($this->failure_order_recovery_service->getState(), 'retry_count', 0),
                ],
    ...
    private function resolveSupportContacts(array $settings, int $language_id, string $locale): array
    {
        ...
        return [
            'working_hours' => (bool) Arr::get($support, 'use_contacts_working_hours', true)
                ...
                : (array) Arr::get($support, "working_hours.$language_id", []),
    ...
    private function normalizeSupportItems(mixed $items, string $locale): array
    {
        return collect(is_array($items) ? $items : [])
            ...
            ->sortBy(fn (array $item): int => (int) Arr::get($item, 'sort_order', 0))
            ->map(fn (array $item): array => [
                    'value' => Str::trim((string) Arr::get($item, 'value')),
                    'type' => (string) Arr::get($item, 'type', 'mobile'),
                    'custom_css_classes' => Str::squish((string) Arr::get($item, 'custom_css_classes', '')),
                    'label' => (string) Arr::get($item, 'value'),
                    'locale' => $locale,
                ])
            ->values()
            ->all();
    ...
    private function resolveImages(mixed $rows): array
    {
        return collect(is_array($rows) ? $rows : [])
            ...
            ->sortBy(fn (array $row): int => (int) Arr::get($row, 'sort_order', 0))
    ...
    private function resolvePaymentMethods(string $locale): array
    {
        ...
        return collect($methods)
            ...
            ->map(function (array $method) use ($locale): array {
                $payment_name = (string) Arr::get($method, 'payment_name', '');
                $translation_key = (string) Arr::get($method, 'label_translation_key', '');

                if ($payment_name === '' && $translation_key !== '') {
                    $payment_name = (string) Lang::get($translation_key, [], $locale);
                }

                return [
                    'payment_method' => (string) Arr::get($method, 'payment_method', ''),
