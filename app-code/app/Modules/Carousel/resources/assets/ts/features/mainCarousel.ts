...
import { findArrayElems, fromJson, getDataset, isEmpty, isNaNValue, parseInteger } from '@ts-shared/lib/helpers.ts';
...
function hasEnoughSlidesForCarousel(carouselElement: HTMLElement): boolean {
    const slidesCountFromData: number = parseInteger(getDataset(carouselElement, 'slidesCount') ?? '');
