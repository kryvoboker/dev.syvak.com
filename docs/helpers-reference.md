...
| `get_slug_variants(?string $sluggable_type, ?string $slug_value = null, ?string $variant_slug_value = null, array $attribute_filters = []): array` | Builds localized product/variant links for language switching. A default variant without its own localized slug uses the localized parent product slug; when no SEO pair exists, the result contains the static product-variant route and IDs. |
| `resolve_product_variant_slug_variants(string $slug_value, ?string $variant_slug_value, array $language_ids_by_code, array $attribute_filters, Slug $slug_instance): array` | Resolves product and variant slug pairs used by localized product links, with a static ID-based fallback for variants that cannot be represented by localized SEO slugs. |
...
localized.catalog.product.static.show   // /{locale}/product/static/{product_id}/{variant_id}
```
...
The static product-variant route is generated by `get_slug_variants()` only when the target locale does not have enough SEO data to resolve the product safely. It is handled by `ProductController::showStatic()` and still validates that the variant belongs to the requested product.
