...
const fetchFunc = async (url : string, data : FetchFuncOptions | FormData = {}, method : string = 'POST', isRecursion : boolean = false) : Promise<any> => {
    ...
    if (!response.ok && !isRecursion) {
        ...
        // TODO: need dev common modal func
        /*await import('@catalogUtils/commonModal.ts')
         .then(module => module.handleShowCommonModal(message))
         .catch(error => showErrorInConsole(error.message));*/

        await fetchFunc(
            location.origin + '/index.php?route=ajax_exception',
            formData,
            'POST',
            true
        );
